...
sri_data <- read_csv(here::here(c("data/othersites/channel_islands/SOCAL_W_01_WW_encounterTimes_Ziphius_cavirostris.csv", 
...
#double check that this method is okay for this dataset! 
sri_effort <- sri_data %>%
  mutate(date = dmy_hms(endEnc), 
         month = month(date),
         year = year(date)) %>% 
  mutate(sampled = TRUE)
...
sri_bw <- sri_data %>% 
  ...
  summarize(total_mins = sum(mins), 
            avg_mins = mean(mins)) %>% 
  ungroup() %>% 
  complete(year = 2021:2023, month = 1:12, fill = list(total_mins = 0, avg_mins = NA)) %>% 
